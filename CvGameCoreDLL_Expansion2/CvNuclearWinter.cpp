...
bool CvNuclearWinterLevel::CacheResults(Database::Results& kResults, CvDatabaseUtility& kUtility)
{
	...
	{
		std::string strKey("NuclearWinterLevel_GlobalYieldRate");
		Database::Results* pResults = kUtility.GetResults(strKey);
		if(pResults == NULL)
		{
			pResults = kUtility.PrepareResults(strKey, "select Yields.ID as YieldID, Yield from NuclearWinterLevel_GlobalYieldRate inner join Yields on Yields.Type = YieldType where NuclearWinterLevelType = ?");
		}
...
int CvNuclearWinterLevel::GetYieldMinThreshold(int iYieldIndex) const
{
