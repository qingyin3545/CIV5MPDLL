...
public:
	inline CvEnumMap()
		: m_values(NULL)
	{
		CvAssert_Debug(sizeof(StorageType) == sizeof(T));
		CvAssert_Debug(__alignof(StorageType) == __alignof(T));
	}
	inline ~CvEnumMap()
	{
		uninit();
	...
	void init()
	{
		CvAssert_Debug(m_values == NULL, "Dynamic CvEnumMap<>::init() called without first calling CvEnumMap<>::uninit() - Memory will leak");
	...
	{
		checkValidAccess();
		CvAssert_Debug(idx < size(), "Attempting to access out of range index in CvEnumMap<>");
		return data()[idx];
	}
...
private:
	...
	{
		CvAssert_Debug(m_values != NULL, "Attempting to access dynamic CvEnumMap<> without first calling CvEnumMap<>::init()");
#ifdef VPDEBUG
		CvAssert_Debug(m_values == NULL || m_initSize == size(), "Attempting to access dynamic CvEnumMap<> whose size has changed since CvEnumMap<>::init() was called");
#endif
	}
...
public:
	...
	void init()
	{
		CvAssert_Debug(m_initialized == false, "Fixed CvEnumMap<>::init() called without first calling CvEnumMap<>::uninit() - Elements will not be destroyed");
		m_initialized = true;
		for (Iterator it = begin(); it != end(); ++it)
		{
			new (&(*it)) T;
...
private:
	...
	inline void checkValidAccess() const
	{
		CvAssert_Debug(m_initialized == true, "Attempting to access fixed CvEnumMap<> without first calling CvEnumMap<>::init()");
	}
