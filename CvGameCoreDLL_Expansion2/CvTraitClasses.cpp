...
bool CvTraitEntry::CacheResults(Database::Results& kResults, CvDatabaseUtility& kUtility)
{
	...
	//CityYieldPerAdjacentFeature
	{
		...
		if(pResults == NULL)
		{
			pResults = kUtility.PrepareResults(strKey, "select Features.ID as FeatureID, Yields.ID as YieldID, MaxValue from CityYieldPerAdjacentFeature inner join Features on Features.Type = FeatureType inner join Yields on Yields.Type = YieldType where TraitType = ?");
		}
		...
		while(pResults->Step())
		{
			...
			m_ppiCityYieldPerAdjacentFeature[FeatureID][YieldID] = yield;
